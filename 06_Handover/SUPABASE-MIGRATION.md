...
- **Day 3 — DONE 2026-07-19:** Supabase project ref `drgffhoigdocwbusincp`, URL `https://drgffhoigdocwbusincp.supabase.co`. DDL executed in dashboard SQL editor: Success, no rows returned. Five tables + RLS + own-rows policies live.
- **Day 4 — PARTIAL (P0 conditionally closed):** Region `us-east-1` (t3.nano). RLS enabled + own-rows policies confirmed created by DDL success. Anon REST read `GET /rest/v1/prs?select=*` with publishable key returned `[]` — no leak. **Deferred to Day 7:** seeded-row proof (anon cannot read/update a row belonging to a real user). Dashboard role-impersonation abandoned — unworkable on iPhone Safari, and the FK to `auth.users` rejects fabricated-UUID inserts before RLS is ever evaluated, so the proof requires a real authed user, which Day 7 produces naturally. **HARD BLOCKER:** must pass before Day 12 dual-write opens any write path. Deferral risk is nil — Supabase is a write-only shadow through Phase 3 with localStorage authoritative.
- **Day 5 — DONE 2026-08-05:** `@supabase/supabase-js@^2.112.1` installed; `src/supabase.js` created. Exports `supabase` (null when env absent) + `sbReady()`; `window.igSbStatus()` console probe. Auth opts: persistSession, autoRefreshToken, detectSessionInUrl, storageKey `ig_sb_auth`. Nothing imports it — bundle unchanged at 309.04 kB (tree-shaken out), zero production surface. **Netlify env vars still to be set** before Day 6 does anything useful.
- **Day 6 — DONE 2026-08-05:** `src/AuthTest.jsx` — magic-link send + session display (user UUID, email, sign out), plus an explicit NOT-CONFIGURED state when env vars are missing. Gated in `src/main.jsx` on `?authtest=1`. **`AgentTrainer.jsx` untouched** — the flag is read at the entry point, so no session-screen edit and no SMOKE_TEST §5 required. **Lazy-loaded via `React.lazy`**: a static import put supabase-js in the main chunk (309 → 531 kB). Split restores main to 310.26 kB with AuthTest's 222 kB in its own chunk, fetched only under the flag. `smoke.mjs` patched to strip the entry chunk's trailing `export{...}` — code-split output is invalid in `window.eval`'s script context; harness limitation, not an app defect.
- **Day 7:** Auth round-trip proof: send link, click, session returns user UUID. Persist session via supabase-js defaults. **Plus the carried-forward Day 4 RLS proof:** with a real authed user, seed one `prs` row as that user, then verify anon (publishable key, no JWT) gets 0 rows on read and 0 rows affected on update. P1 gate + deferred P0 assertion.
